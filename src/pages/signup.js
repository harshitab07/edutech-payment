      ...
      <div className="w-screen flex justify-center mt-4">
        ...
        <div className="px-2 flex flex-col items-center gap-2">
          <div className="flex w-9/12 flex-col gap-1">
            ...
            <input className="py-2 px-4 rounded-md focus:outline-none" type="text" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="flex w-9/12 flex-col gap-1">
          ...
          <button className="p-3 rounded-md bg-[#AEBFFA] text-center w-40" onClick={handleSignUp}>Sign Up</button>
          <p className="mb-3">Already a member? <Link to="/sign-in" className="underline cursor-pointer">Sign In</Link></p>
        </div>
        </div>
      </div>
    </div>
